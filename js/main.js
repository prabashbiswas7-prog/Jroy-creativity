...
const PRODUCTS = [
  // Earrings
  {
    ...
    rating: 5,
    reviews: 51,
    image: 'https://images.unsplash.com/photo-1599643478514-4a734639b740?w=500&q=80',
    badge: 'New',
    icon: '◈',
    bg: '#f1e4e4',
  ...
  // Rings
  {
    ...
    rating: 5,
    reviews: 11,
    image: 'https://images.unsplash.com/photo-1573408301145-b98c4af06b86?w=500&q=80',
    badge: 'Popular',
    icon: '○',
    bg: '#ede8f5',
  ...
  },
  {
    ...
    rating: 4,
    reviews: 37,
    image: 'https://images.unsplash.com/photo-1610815525547-49f3e49e6231?w=500&q=80',
    badge: 'New',
    icon: '⊕',
    bg: '#f5ead8',
...
function renderStars(rating, reviews) {
  ...
  return `<div class="product-rating"><span class="stars">${stars}</span> <span class="reviews">(${reviews} review${reviews > 1 ? 's' : ''})</span></div>`;
}
...
function renderProductCard(p) {
  ...
  <a href="product.html?id=${p.id}" class="product-card">
    <div class="product-card-img">
      <img src="${p.image}" alt="${p.name}" class="product-img-tag">
    ...
    <div class="product-card-body">
      ...
      <div class="product-footer">
        <div class="product-pricing">
          <span class="product-price-old">₹${p.originalPrice.toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
          <span class="product-price">₹${p.price.toLocaleString('en-IN', {minimumFractionDigits: 2})}</span>
        </div>
        <a href="${JROY.waLink('Hi! I\'m interested in ' + p.name)}" 
           target="_blank" class="product-wa-btn" onclick="event.stopPropagation()"
           title="Order on WhatsApp">${waSVG()} Order on WhatsApp</a>
      </div>
    </div>
  </a>`;
}
